...
![User Journey Flowchart Part 1](./image_1764889176296.png)

*(Continued below)*

![User Journey Flowchart Part 2](./image_1764889185743.png)
...
![Three-Tier Architecture](./image_1764889207389.png)
...
![WebSocket Communication Flow](./image_1764889235769.png)

**Figure 16:** WebSocket communication flow showing bidirectional real-time messaging between client browsers and the Railway backend server.

**Explanation:**

This diagram illustrates the complete WebSocket communication lifecycle that powers Nexus's real-time features.

**Step 1: Connection Establishment** - When a user opens the Nexus platform, their browser initiates an HTTP Upgrade request to the Railway server. The server responds with "101 Switching Protocols," establishing a persistent WebSocket (WSS) connection over TCP. This persistent connection eliminates the overhead of repeated HTTP requests, enabling instant bidirectional communication.

**Step 2: Match Broadcast Flow** - When Player A creates a new match request through the REST API (`POST /api/matches`), the server first stores the match in the PostgreSQL database. Immediately after, the server broadcasts a `MATCH_POSTED` event through WebSocket to all connected clients. Players B, C, and others receive this notification in under 100 milliseconds, and their UI updates instantly to show the new match opportunity.

**Step 3: Message Types Supported** - The WebSocket server handles four primary event types:
- `MATCH_POSTED`: Broadcasts new match requests to all connected users
- `PLAYER_ACCEPTED`: Notifies when a match creator accepts a player's application
- `VOICE_CREATED`: Signals that a voice channel has been created for team coordination
- `CONNECTION_REQ`: Alerts match creators when players apply to their matches

**Performance Characteristics:**
- **Latency:** Sub-100ms message delivery from server to all connected clients
- **Reliability:** Automatic reconnection logic handles network interruptions gracefully
- **Efficiency:** Single persistent connection replaces multiple polling requests
...
![Deployment Architecture](./image_1764889249668.png)
